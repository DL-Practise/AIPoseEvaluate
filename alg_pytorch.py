...
class AlgPytorch(threading.Thread):
    ...
    def add_img(self, info):
        if self.img_queue.full():
            return
        else:
            self.img_queue.put(info)
            

    def run(self):
        while self.thread_flag:
            try:
                info = self.img_queue.get(block=True, timeout=1)
                print('alg processing thread: get a info')
            except queue.Empty:
                info = None
                print('alg processing thread: get no info')
            
            if info is not None and self.alg_api is not None:     
                start_time = time.time()
                ret = self.alg_api.inference(info[0])
                if self.cb_func is not None:
                    self.cb_func(info[0], info[1], ret, time.time()-start_time)
            ...
            if self.alg_api is None and self.alg_dir != '' and self.alg_name != '' and self.alg_dev != '':
                ...
                self.cb_func(None, None, ret, None)
